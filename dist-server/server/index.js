...
app.get('/api/sas', async (req, res) => {
    try {
        ...
        const permissions = req.query.permissions || 'w'; // Default to write only
        if (!fileName) {
...
app.get('/api/transfer/:id', async (req, res) => {
    try {
        ...
        console.log(`[TRANSFER] ID: ${transferId}, Password header: ${!!password}`);
        ...
        if (error.statusCode === 401) {
            console.log(`[TRANSFER] Sending 401 for transfer ${req.params.id}`);
